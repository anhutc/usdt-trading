...
@st.cache_data(ttl=600) # Cache for 10 minutes, as candle data might update
def filter_doji_volume(pair, exchange_name, num_doji_candles, doji_candle_timeframe, doji_body_percentage, avg_volume_candles, doji_calculation_method):
    try:
        ...
        for i in range(-num_doji_candles, 0): # Use num_doji_candles
            ...
            # Doji condition
            if body_percentage < doji_body_percentage: # Use doji_body_percentage
                doji_found = True
                doji_volume = volume
                break
        ...
        if doji_found:
            ...
            average_volume = df['volume'].iloc[-avg_volume_candles:].mean() # Use avg_volume_candles
...
def perform_filtering(selected_exchanges, exclude_leverage_tokens, exclude_futures_tokens, max_results_input, status_placeholder, progress_placeholder):
    ...
    try:
        ...
        st.session_state.selected_pair_data = None
        st.session_state.selected_pair_index = None
        st.session_state.stop_filtering = False # Ensure stop_filtering is reset
        st.session_state.stop_button_disabled_temp = False # Ensure stop_button_disabled_temp is reset
...
with col2:
    if st.session_state.filtering_in_progress:
        if st.button(
            ...
            disabled=not st.session_state.filtering_in_progress or st.session_state.stop_button_disabled_temp,
